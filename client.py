...
# 登陆窗口
root1 = tkinter.Tk()
root1['background']='LightPink'
root1.title('登录')
root1['height'] = 200
root1['width'] = 370

IP1 = tkinter.StringVar()
IP1.set('127.0.0.1:50007')  # 默认显示的ip和端口（127.0.0.1位默认本机地址）
User = tkinter.StringVar()
User.set('')


# 用户名标签
labelUser = tkinter.Label(root1, text='please input your name')
labelUser.place(x=80, y=50, width=180, height=30)

entryUser = tkinter.Entry(root1, width=80, textvariable=User)
entryUser.place(x=80, y=90, width=180, height=30)


# 登录按钮
def login(*args):
    global IP, PORT, user
    IP, PORT = '127.0.0.1:50007'.split(':')  # 获取IP和端口号
...
but.place(x=150, y=160, width=70, height=30)
...
def users():
    ...
    if ii == 1:
        ...
        listbox1.place_forget()  # 隐藏在线用户这一栏
        ii = 1
